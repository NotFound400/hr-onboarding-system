...
- [ ] **唯一入口验证**:
    - [ ] 尝试直接访问 `/register`（不带 Token），应报错或跳转登录页。
    - [ ] [cite_start]只有通过 HR 生成的链接（如 `/register?token=xyz`）才能进入 [cite: 59-60]。
- [ ] **账户创建**:
    - [ ] [cite_start]必须输入: `Username`, `Password`, `Email` [cite: 61]。
    - [ ] [cite_start]Email 和 Username 必须唯一 [cite: 61]。

#### 1.2 登录流程 (Login)
[cite_start]*参考: Section 2 & HR-1* [cite: 62-64, 336-340]

- [ ] [cite_start]**未登录拦截**: 访问 `/employee/home` 等受保护路由，必须强制跳回 `/login` [cite: 64]。
- [ ] **员工路由**:
    - [ ] [cite_start]员工登录后，必须**重定向到 Personal Information 页面** (`/employee/profile`)，而不是主页 [cite: 63]。
- [ ] **HR 路由**:
    - [ ] [cite_start]HR 登录后，必须**重定向到 HR Home 页面** [cite: 342]。

---

### 📝 2. Onboarding 表单 (Onboarding)

*前置条件: 使用 `New Hire` 账号登录*
[cite_start]*参考: Section 3* [cite: 65-109]

#### 2.1 个人信息字段逻辑
- [ ] [cite_start]**Email 字段**: 必须预填充注册时的 Email，且**不可编辑** [cite: 80]。
- [ ] [cite_start]**头像 (Avatar)**: 如果用户未上传，必须显示**默认图片** [cite: 76-77]。
- [ ] [cite_start]**身份逻辑 (Citizenship Logic)** [cite: 82-88]:
    - [ ] 选择 "Citizen" 或 "Green Card": **不应**显示 Work Authorization 部分。
    - [ ] 选择 "No": 必须显示 Work Auth 下拉框 (H1-B, L2, F1, H4, Other)。
        - [ ] [cite_start]选择 "Other": 必须显示 **Work Auth Type 输入框** + Start Date + End Date [cite: 84]。
        - [ ] [cite_start]选择其他 (如 F1): 显示 Start Date + End Date [cite: 85]。
        - [ ] [cite_start]**强制上传**: 所有非公民必须上传 Work Auth 文件 (EAD, H1B doc 等) [cite: 86-88]。
- [ ] [cite_start]**驾照逻辑 (Driver's License)** [cite: 89-92]:
    - [ ] 选择 "No": 隐藏相关字段。
    - [ ] 选择 "Yes": 显示 License Number, Expiration Date, Upload Copy。
- [ ] [cite_start]**推荐人 (Reference)** [cite: 93-94]:
    - [ ] **数量限制**: 只能填写 **1 个** 推荐人。
    - [ ] **字段完整性**: 必须包含 Name, Phone, **Address**, Email, Relationship。
- [ ] [cite_start]**紧急联系人 (Emergency Contact)** [cite: 95-96]:
    - [ ] **数量要求**: 必须至少填写 **1 个**，允许添加多个。

#### 2.2 文档与提交逻辑
- [ ] **文档页面**:
    - [ ] [cite_start]必须列出所有数字文档 (Digital Documents) [cite: 97]。
    - [ ] [cite_start]**预览**: 点击文档，弹出 `<object>` 预览窗口 [cite: 102]。
    - [ ] [cite_start]**下载**: 每个文档必须有下载链接 [cite: 99]。
- [ ] **提交状态**:
    - [ ] [cite_start]点击 Submit 后，页面显示 "Please wait for HR to review..." [cite: 107]。
    - [ ] [cite_start]此时**禁止**访问主页 (Home Page) [cite: 107]。
...
- [ ] [cite_start]**导航项** [cite: 112-119]:
    ...
    - [ ] Visa Status Management (**仅非公民可见**)
    - [ ] Housing
    - [ ] House Detail
    - [ ] Report Facility Issue
- [ ] [cite_start]**Hover 特效**: 鼠标悬停在 "Visa Status Management" 上时，显示 "OPT STEM Management" 链接 [cite: 115-116]。

#### 3.2 个人信息 (Personal Info)
- [ ] [cite_start]**布局检查**: 是否包含 Employment, Contact, Emergency Contact 等板块 [cite: 236-263]。
- [ ] [cite_start]**SSN 显示**: 仅显示**后 4 位** [cite: 242]。
- [ ] [cite_start]**编辑/取消 (Edit/Cancel)** [cite: 264-269]:
    - [ ] 点击 Edit 后，按钮变为 Save 和 Cancel。
    - [ ] 点击 Cancel 时，**必须弹出 Alert**: `"Are you sure to discard all your changes?"` (文案必须完全一致)。
- [ ] [cite_start]**文档列表**: 按 `createdDate` **倒序**排列 (最新的在最上面) [cite: 276]。
...
- [ ] **流程顺序 (Flow)**: 验证以下顺序是否强制执行：
    1.  下载 I-983 (必须填)。
    2.  上传 I-20 (上传后才能进行下一步)。
    3.  上传 OPT STEM Receipt。
    4.  上传 OPT STEM EAD。
...
#### 4.1 仪表盘 (Home)
- [ ] [cite_start]**待办事项 (Tracking Table)** [cite: 350-358]:
    - [ ] 是否显示需要 HR 操作的任务 (如 Approve Visa, Review Onboarding)？
    - [ ] 字段: Name, Type of Application, Status, Last Modification Date。

#### 4.2 员工档案 (Employee Profile)
- [ ] [cite_start]**分页显示**: 必须显示当前记录位置，格式为 **`<10/100>`** [cite: 368-370]。
- [ ] [cite_start]**搜索栏** [cite: 372-374]:
    - [ ] 必须支持搜 First Name **或** Last Name **或** Preferred Name。
    - [ ] 测试：搜不到时显示 "No record found"。

#### 4.3 招聘与 Token (Hire)
- [ ] **生成 Token**:
    - [ ] 输入 Email，生成链接。
    - [ ] [cite_start]**有效期**: 默认必须是 **3 小时** [cite: 398]。
- [ ] **审核 Onboarding**:
...
- [ ] [cite_start]**列表视图** [cite: 429-435]:
    - [ ] 显示 Address, # of Employees, Landlord Info。
- [ ] **详情视图**:
    - [ ] [cite_start]**设施计数**: 显示 Beds, Mattresses, Tables, Chairs 的数量 [cite: 444-447]。
    - [ ] [cite_start]**报修列表分页**: **每页只显示 3-5 条报告** (Strict Requirement) [cite: 451]。
    - [ ] [cite_start]**报修详情**: 点击报告，弹窗显示详细信息和评论历史 [cite: 453-463]。
